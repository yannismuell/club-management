...
/**
 * Defines v1 of the project app API, which provides simple methods.
 */
@Api(
        name = "qrcool",
        version = "v1",
...
public class ProjectAPI {
    ...
    private static final Logger LOG = Logger.getLogger(ProjectAPI.class.getName());
    ...
    private static Account getAccountFromUser(User user, String userId) {
        ...
        if (account == null) {
            ...
            float restTime = 11;
            account = new Account(userId, extractDefaultDisplayNameFromEmail(email), "", "", email);
        }
        return account;
    }
    ...
    public Account saveAccount(final User user, final AccountForm accountForm) throws Exception {
        ...
        Account account = ofy().transact(new Work<Account>() {
            ...
            public Account run() {
                ...
                if (account == null) {
                    String email = user.getEmail();
                    account = new Account(getUserId(user), firstName, surName, companyName, email);
                } else {
                    account.update(getUserId(user), firstName, surName, companyName, mainEmail);
                }
